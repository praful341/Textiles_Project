...
namespace Textiles_Project.Class
{
    static class Global
    {
        ...
        public static void Export(string format, GridView gvExportGrid, bool isHeaderPrint = true)
        {
            try
            {
                ...
                if (format.Equals(Exports.xls.ToString()))
                {
                    if (Global.Confirm("Export Done\n\nYou Want To Open Excel File ?", "Mahakal_Consultancy", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == System.Windows.Forms.DialogResult.Yes)
                    {
                ...
                {
                    if (Global.Confirm("Export Done\n\nYou Want To Open PDF File ?", "Mahakal_Consultancy", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == System.Windows.Forms.DialogResult.Yes)
                    {
                        System.Diagnostics.Process.Start(Filepath);
                    }
                }
        ...
        public static void LOOKUPParty(LookUpEdit lookup)
        {
        ...
        {
            PartyMaster objBroker = new PartyMaster();
            DataTable Broker = objBroker.Broker_GetData(1);
            lookup.Properties.DataSource = Broker;
            lookup.Properties.ValueMember = "broker_id";
            lookup.Properties.DisplayMember = "broker_name";
            lookup.ClosePopup();
        }
        public static void LOOKUPTransport(LookUpEdit lookup)
        {
            TransportMaster objTransport = new TransportMaster();
            DataTable Transport = objTransport.GetData(1);
            lookup.Properties.DataSource = Transport;
            lookup.Properties.ValueMember = "transport_id";
            lookup.Properties.DisplayMember = "transport_name";
            lookup.ClosePopup();
        }
        public static void LOOKUPItemRep(DevExpress.XtraEditors.Repository.RepositoryItemLookUpEdit lookup)
        {
            ...
            DataTable Item = objItem.GetData(1);
            lookup.DataSource = Item;
            lookup.ValueMember = "item_id";
            lookup.DisplayMember = "item_name";
        }
        public static void LOOKUPGoDownRep(DevExpress.XtraEditors.Repository.RepositoryItemLookUpEdit lookup)
        {
            GoDownMaster objGoDown = new GoDownMaster();
            DataTable GoDown = objGoDown.GetData(1);
            lookup.DataSource = GoDown;
            lookup.ValueMember = "godown_id";
            lookup.DisplayMember = "godown_name";
        }
        ...
        public static void LOOKUPCustomer(LookUpEdit lookup)
        {
            CustomerMaster objCustomer = new CustomerMaster();
            DataTable Customer = objCustomer.GetData();
            lookup.Properties.DataSource = Customer;
            lookup.Properties.ValueMember = "customer_id";
            lookup.Properties.DisplayMember = "customer_name";
            lookup.ClosePopup();
        }
        public static void LOOKUPBankName(LookUpEdit lookup)
        {
            CreditCardMaster objBank = new CreditCardMaster();
            DataTable Bank = objBank.Distinct_Bank_GetData();
            lookup.Properties.DataSource = Bank;
            lookup.Properties.ValueMember = "bank_name";
            lookup.Properties.DisplayMember = "bank_name";
            lookup.ClosePopup();
        }
        public static void LOOKUPMachineName(LookUpEdit lookup)
        {
            MachineMaster objMachine = new MachineMaster();
            DataTable Machine = objMachine.GetData_Search();
        ...
        public static void LOOKUPCreditCardCustomer(LookUpEdit lookup)
        {
            CustomerMaster objCustomer = new CustomerMaster();
            DataTable Customer = objCustomer.GetData();
